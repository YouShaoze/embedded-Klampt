/*
* @File Name CollisionMesh.cpp
* @File Path M:\MAS2\embedded Klampt\KrisLibrary\geometry\CollisionMesh.cpp
* @Author: Ruige_Lee
...
void RigidTransformToPQP(const RigidTransform& f,double R[3][3],double T[3])
{
...
void PQPToRigidTransform(const double R[3][3],const double T[3],RigidTransform& f)
{
	for ( uint8_t i = 0; i < 3; i++ )
	{
		for ( uint8_t j = 0 ; j < 3; j++ )
...
{
	const double* P1 = pqpResults->distance.P1();
	const double* P2 = pqpResults->distance.P2();
...
{
	const double* P1 = pqpResults->tolerance.P1();
	const double* P2 = pqpResults->tolerance.P2();
	p1.set(P1[0],P1[1],P1[2]);
	p2.set(P2[0],P2[1],P2[2]);
}
...
inline void Copy(const double p[3],Vector3& x)
{
	x.set(p[0],p[1],p[2]);
}

inline void Copy(const Vector3& x,double p[3])
{
	p[0] = x.x;
	p[1] = x.y;
	p[2] = x.z;
...
int aabb_sphere_disjoint(const double box_dims[3],const double c[3], double r)
{
	double c_cropped[3];
...
Real aabb_ray_collide(const double box_dims[3],const double s[3],const double d[3])
{
	AABB3D b;
...
void ToLocal(const BV& b,const double p[3],double ploc[3])
{
	double psub[3];
	VmV(psub,p,b.To);
	MTxV(ploc,b.R,psub);
...
inline bool CollideBV(const double d[3],const Vector3& pt)
{
	return Abs(pt.x)<=d[0] && Abs(pt.y)<=d[1] && Abs(pt.z)<=d[2];
}

inline void DistanceLimitsBV(const double d[3],const Vector3& pt,Real& dmin,Real& dmax)
{
	double p[3];
...
inline bool CollideBV(const double d[3],const Segment3D& s)
{
...
inline bool CollideBV(const double d[3],const Sphere3D& s)
{
	double dmin,dmax;
...
inline bool CollideBV(const double d[3],BV& bv)
{
	double temp[3];
	VcV(temp,d);
...
{
	double S0[3],D0[3],S[3],D[3],temp[3];
	Copy(r.source,S0);
	Copy(r.direction,D0);
...
inline
double
TriDistance(double R[3][3], double T[3], Tri *t1, Tri *t2,
						double p[3], double q[3])
{
	...
	double d=TriDist(p,q,tri1,tri2);
