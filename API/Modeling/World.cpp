/*
* @File Name World.cpp
* @File Path M:\MAS2\embedded Klampt\API\Modeling\World.cpp
* @Author: Ruige_Lee
...
string RobotWorld::GetName(int id) const
{
	...
	if( j.first >= 0 )
	{
		return robots[j.first]->name+"["+robots[j.first]->linkNames[j.second]+"]";
	}
	return "";
}
...
{
	if(id >= (int)terrains.size() && id < (int)(terrains.size()+rigidObjects.size()))
	{
		return id-(int)terrains.size();
	}
	return -1;
}
