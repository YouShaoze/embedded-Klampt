/*
* @File Name RigidObject.cpp
* @File Path M:\MAS2\embedded Klampt\API\Modeling\RigidObject.cpp
* @Author: Ruige_Lee
* @Date:   2019-05-19 11:47:44
* @Last Modified by:   Ruige_Lee
* @Last Modified time: 2019-05-19 13:46:18
...
bool RigidObject::Load(const char* fn)
{
	...
	if(ext && strcmp(ext,"obj")==0)
	{
		...
		if(!f.CheckSize("mesh",1,fn))
		{
			return false;
		}
		if(!f.CheckType("mesh",PrimitiveValue::String,fn))
		{
			return false;
		}
		...
		{
			if(!f.CheckType("geomscale",PrimitiveValue::Double,fn))
			{
				return false;
			}
			...
			if(scale.size()==1)
			{
				geomT(0,0)=geomT(1,1)=geomT(2,2)=scale[0];
			}
		...
		{
			if(!f.CheckType("geomtranslate",PrimitiveValue::Double,fn))
			{
				return false;
			}
			if(!f.CheckSize("geomtranslate",3,fn))
			{
				return false;
			}
		...
		{
			if(!f.CheckType("T",PrimitiveValue::Double,fn))
			{
				return false;
			}
			vector<double> items = f.AsDouble("T");
			if(items.size()==12)
			{ //read 4 columns of 3, row major then translation, just like RigidTransform
			...
			{ //read 4 rows of a 4 x 4 transform matrix
		...
		{
			if(!f.CheckType("position",PrimitiveValue::Double,fn))
			{
				return false;
			}
			if(!f.CheckSize("position",3,fn))
			{
				return false;
			}
		...
		{
			if(!f.CheckType("velocity",PrimitiveValue::Double,fn))
			{
				return false;
			}
			if(!f.CheckSize("velocity",3,fn))
			{
				return false;
			}
		...
		{
			if(!f.CheckType("angularVelocity",PrimitiveValue::Double,fn))
			{
				return false;
			}
			if(!f.CheckSize("angularVelocity",3,fn))
			{
				return false;
			}
		...
		{
			if(!f.CheckSize("mass",1))
			{
				return false;
			}
			if(!f.CheckType("mass",PrimitiveValue::Double))
			{
				return false;
			}
		...
		{
			if(!f.CheckSize("com",3))
			{
				return false;
			}
			if(!f.CheckType("com",PrimitiveValue::Double))
			{
				return false;
			}
			hasCOM = true;
		...
		if (f.count("inertia")==0)
		{
			inertia.setIdentity();
		}
		else
		{
			if(!f.CheckType("inertia",PrimitiveValue::Double,fn))
			{
				return false;
			}
			...
			if(items.size() == 3)
			{
				inertia.setDiagonal(Vector3(items[0],items[1],items[2]));
			}
		...
		{
			if(!f.CheckSize("kFriction",1,fn))
			{
				return false;
			}
			if(!f.CheckType("kFriction",PrimitiveValue::Double,fn))
			{
				return false;
			}
		...
		if(f.count("kRestitution")==0)
		{
			kRestitution = 0.5;
		}
		else
		{
			if(!f.CheckSize("kRestitution",1,fn))
			{
				return false;
			}
			if(!f.CheckType("kRestitution",PrimitiveValue::Double,fn))
			{
				return false;
			}
		...
		if(f.count("kStiffness")==0)
		{
			kStiffness=Inf;
		}
		else
		{
			if(!f.CheckSize("kStiffness",1,fn))
			{
				return false;
			}
			if(!f.CheckType("kStiffness",PrimitiveValue::Double,fn))
			{
				return false;
			}
